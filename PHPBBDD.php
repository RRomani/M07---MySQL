...
<body>
	...
	<?php
	if(isset($selector)){
		$selector=$_POST['COUNTRY'];
	}else{
		$selector = "ESP";			
	}

 		# (1.1) Connectem a MySQL (host,usuari,contrassenya)
	$conn = mysqli_connect('localhost','root','Riicardo29!');

 		# (1.2) Triem la base de dades amb la que treballarem
	mysqli_select_db($conn, 'world');

 		# (2.1) creem el string de la consulta (query)
	$selector =$_POST['CountryCode'];
	$consulta2 = "SELECT * FROM city where CountryCode =  '".$selector."'";
	...
	if (!$resultat) {
		...
		$message .= 'Consulta realitzada: ' . $consulta2;
		die($message);
	}
	?>
